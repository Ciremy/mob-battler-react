...
useEffect(() => {
  ...
  return (
    ...
    <Label>
        ...
        <Input type='text' value={attack.name} onChange={(event)=> {
            handleChange(event.target.value, "name")}}/>
    <Label>
        Damage
    </Label>
        <Input type='number' value={attack.damage} onChange={(event)=> {
            handleChange(parseStringToNumber(event.target.value), "damage")}}/>
    <Label>
        Mana Cost
    </Label>
        <Input type='number' value={attack.manaCost} onChange={(event)=> {
            handleChange(parseStringToNumber(event.target.value), "manaCost")}}/>
    <Label>
        Type
    </Label>
        <Input type='select' value={attack.attackType}onChange={(event)=> {
            handleChange(event.target.value, "attackType")}}>
            {ATTACK_TYPES.map(type => <option key={type}>{type}</option> )}
        </Input>
    </Form>
    <Label>
        Image Url
